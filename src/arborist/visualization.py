...
def plot_curve_mip(
    ...
    """
    # Project 3D curves
    curve1, curve2 = geometry.curves_pca_projection(curve1, curve2)
...
def _add_error_annotation(fig, ax, curve1, curve2):
    ...
    """
    # Compute error
    gt_length = geometry.path_length(curve1)
    recon_length = geometry.path_length(curve2)
    error = geometry.max_l2_error(curve1, curve2)
...
def plot_latents_by_pca(curves, latents, output_path=None):
    ...
    lengths = np.array([geometry.path_length(c) for c in curves])
...
def _plot_latents_by_direction(curves, latents_2d, pca, output_path=None):
    ...
    directions = [geometry.curve_principal_direction(c) for c in curves]
